...
export default function DisputePage() {
    ...
    const extractDisputedItems = (orders, sellerId) => {
        if (!orders) return [];
            ...
            (order.items || [])
                .filter(
                    (item) =>
                        ...
                        ["DISPUTING", "RESOLVED"].includes(item.status)
                )
                .map((item) => ({
                    ...item,
    ...
    useEffect(() => {
        ...
        const fetchDisputedOrders = async () => {
            try {
                ...
                const disputes = extractDisputedItems(res.data, userId);
    ...
    const handleResolved = async (orderId, productId) => {
        try {
            await axios.put(`${API_BASE_URL}/orders/item/resolve`, { orderId, productId });
    ...
    // ✅ Extracted ternary logic into helper
    const getStatusClass = (status) => {
        if (status === "DISPUTING") return "text-red-600";
        if (status === "RESOLVED") return "text-green-600";
        return "text-gray-800";
    };

    // ✅ Render helpers to flatten conditionals
    const renderLoading = () => (
                ...
                <tbody>
                    {disputedItems.map((item) => (
                        ...
                        >
                            ...
                            <td className={`px-4 py-2 font-semibold ${getStatusClass(item.status)}`}>
                                {item.status}
                            </td>
